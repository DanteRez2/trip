(() => {
  ...
  function renderPlannerTools(day) {
    ...
    const dayCost = total(day.items, 'cost');
    const tripTotal = tripEntryBudget + totalHotelBudget();

    return `
      <section class="planner-tools" aria-label="Trip planning tools">
        <div class="tool-card budget-card">
          <div class="tool-heading"><span>Budget</span><div><b>${euro(dayCost)}</b> <small style="display:block;text-align:right">${euro(tripTotal)}</small></div></div>
          <p>Day budget &nbsp; | &nbsp; <small>Trip total below</small></p>
          <div class="budget-progress"><span style="width:${Math.min(100, tripEntryBudget ? (dayCost / tripEntryBudget) * 100 : 0)}%"></span></div>
          <small>${euro(tripTotal)} total estimated trip cost (activities + hotels)</small>
        </div>

        <details class="tool-card hotel-card" ${day.hotelCity ? 'open' : ''}>
          <summary><span>Hotel · ${escapeHtml(city)}</span><b>${hotel.name ? 'Saved' : 'Add details'}</b></summary>
          <div class="tool-body hotel-form" data-city="${escapeHtml(city)}">
            ...
            <div class="hotel-actions">
              <button type="button" class="save-hotel">Save hotel</button><button type="button" class="hotel-options" onclick="window.location.href='hotels.html'">Hotel Options</button>
        ...
        <details class="tool-card reservation-card">
          <summary><span>Reservations</span><b>${completed}/${bookables.length || 0} ready</b></summary>
  ...
  function selectDay(index) {
    ...
    activeIndex = index;
    renderDay();
    if(wasOpen){const d=document.querySelector(".reservation-card"); if(d) d.open=true;}
