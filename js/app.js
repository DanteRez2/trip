(() => {
  ...
  function renderDay() {
    ...
    const activities = day.items.map(item => {
      ...
      lastPeriod = period;
      const journey = item.transport ? `<div class="journey-card"><span class="journey-icon">${/walk/i.test(item.transport) ? '↓' : '🚇'}</span><span>${escapeHtml(item.transport)}</span></div>` : '';
      return `${heading}
        <article class="activity-card" id="activity-${escapeHtml(item.id)}">
          ...
          <div class="activity-content">
            <div class="activity-top">
              ...
              <div class="activity-badges"><span class="core-badge">${item.core ? 'CORE' : 'OPTIONAL'}</span>${item.priority ? `<span class="priority-badge">${escapeHtml(item.priority)}</span>` : ''}</div>
            </div>
            ...
            ${item.address ? `<div class="address-line"><b>Address:</b> ${escapeHtml(item.address)}</div>` : ''}
            <div class="activity-details">
              ${item.tip ? `<div class="tip"><b>Visit tip:</b> ${escapeHtml(item.tip)}</div>` : ''}
              ${item.photoTip ? `<div class="photo-tip"><b>Photography:</b> ${escapeHtml(item.photoTip)}</div>` : ''}
